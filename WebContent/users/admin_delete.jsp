			...
			<c:forEach items="${candidates.rows}" var="row"
				...
				<tr>
					...
					<td>
						...
						<sql:query var="registrations" dataSource="jdbc/N3CLoginTagLib">
							select email,orcid_id,created,unite_user_id from palantir.n3c_user natural left outer join n3c_admin.user_binding where last_name = ? and first_name = ?;
							<sql:param>${row.last_name }</sql:param>
							<sql:param>${row.first_name }</sql:param>
						</sql:query>
							<thead>
							...
							<c:forEach items="${registrations.rows}" var="row2"
								varStatus="rowCounter2">
								<tr><td>${row2.email}</td><td>${row2.orcid_id}</td><td>${row2.created}</td><td>${row2.unite_user_id}</td></tr>
							</c:forEach>
						</table>
					</td>
				</tr>
