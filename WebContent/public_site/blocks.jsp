...
<body>
	...
	<div class="container-fluid" style="padding-left: 5%; padding-right: 5%;">
		...
		<a href="addBlock.jsp"><i class="fas fa-plus-circle fa-2x text-success"></i></a>
		<sql:query var="admins" dataSource="jdbc/N3CLoginTagLib">
 	       select * from n3c_public_site.block order by tag
 	   </sql:query>
		<table border=1>
			<thead>
				<tr>
			...
			<tbody>
				<c:forEach items="${admins.rows}" var="row" varStatus="rowCounter">
					<tr>
						<td><a href="editBlock.jsp?tag=${row.tag}"><i class="fas fa-edit fa-lg text-success"></i></a></td>
						<td>${row.tag}</td>
						<td>${row.block}</td>
						<td>
							<a href="deleteBlock.jsp?tag=${row.tag}"><i class="fas fa-times-circle fa-lg text-danger"></i></a>
						</td>
					</tr>
				</c:forEach>
			</tbody>
